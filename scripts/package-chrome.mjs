...
const manifestPath = path.join(distChromeDir, 'manifest.json');
...
async function getManifestVersion() {
  ...
  if (!manifest.version) {
    throw new Error('dist/chrome/manifest.json is missing a version field');
  }
...
async function main() {
  ...
  const outputZip = path.join(releasesDir, `kampus-auto-login-chrome-v${version}.zip`);

  await mkdir(releasesDir, { recursive: true });
  await rm(outputZip, { force: true });

  // -X strips extra macOS metadata and avoids __MACOSX / AppleDouble files.
  await runCommand('zip', ['-r', '-X', outputZip, '.', '-x', '*/.*', '.*', '__MACOSX/*'], distChromeDir);
